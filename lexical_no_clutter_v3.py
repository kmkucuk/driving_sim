...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='E:\\Backups\\All Files\\Genel\\Is\\2022\\Upwork\\LabX\\studies\\materials\\drivingSimulator\\repo\\lexical_no_clutter_v3.py',
...
textFont = "./stimuli/font/georgia.ttf"
...
for thisBlock in blocks:
    ...
    _key_resp_allKeys = []
    # keep track of which components have finished
    get_trialsComponents = [background_panel_3, block_instruction_text, key_resp]
    ...
    while continueRoutine:
        ...
        if background_panel_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        if background_panel_3.status == STARTED:
            ...
            if tThisFlipGlobal > background_panel_3.tStartRefresh + instructionDuration-frameTolerance:
        ...
        if block_instruction_text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > block_instruction_text.tStartRefresh + instructionDuration-frameTolerance:
                # keep track of stop time/frame for later
                block_instruction_text.tStop = t  # not accounting for scr refresh
                block_instruction_text.frameNStop = frameN  # exact frame index
    ...
    for thisTrial in trials:
        ...
        lexical_text_2.setFont(current_font)
